...
const defaultTestTypes = [
{
  ...
  hasSubtests: false,
  parameters: [
  'Li DM', 'Be DM', 'Mg DM', 'Al DM', 'P DM', 'Ca DM', 'Ti DM', 'V DM', 'Cr DM', 'Mn DM', 'Fe DM', 'Co DM', 'Ni DM', 'Cu DM', 'Zn DM', 'As DM', 'Se DM', 'Sr DM', 'Mo DM', 'Cd DM', 'Sb DM', 'Ba DM', 'Tl DM', 'Pb DM', 'U DM', 'Ag DM', 'Li TM', 'Be TM', 'Mg TM', 'Al TM', 'P TM', 'Ca TM', 'Ti TM', 'V TM', 'Cr TM', 'Mn TM', 'Fe TM', 'Co TM', 'Ni TM', 'Cu TM', 'Zn TM', 'As TM', 'Se TM', 'Sr TM', 'Mo TM', 'Cd TM', 'Sb TM', 'Ba TM', 'Tl TM', 'Pb TM', 'U TM', 'Ag TM', 'B DM', 'Na DM', 'Si DM', 'S DM', 'K DM', 'B TM', 'Na TM', 'Si TM', 'S TM', 'K TM'].
  map((name, i) => ({ id: `P-ICP-${String(i + 1).padStart(3, '0')}`, name, unit: '' })),
  createdBy: 'System',
...
},
{
  ...
  { name: 'SO4', unit: 'ppm' },
  { name: 'PO4', unit: 'ppm' }].
  map((p, i) => ({ id: `P-IC-${String(i + 1).padStart(3, '0')}`, name: p.name, unit: p.unit })),
  createdBy: 'System',
  createdDate: '2024-01-01'
},
{
  ...
  map((p, i) => ({ id: `P-ALK-${String(i + 1).padStart(3, '0')}`, name: p.name, unit: p.unit })),
  createdBy: 'System',
  createdDate: '2024-01-01'
},
{
  ...
  map((p, i) => ({ id: `P-TIC-${String(i + 1).padStart(3, '0')}`, name: p.name, unit: p.unit })),
  createdBy: 'System',
  createdDate: '2024-01-01'
},
{
  ...
  map((p, i) => ({ id: `P-PH-${String(i + 1).padStart(3, '0')}`, name: p.name, unit: p.unit })),
  createdBy: 'System',
  createdDate: '2024-01-01'
}];
...
export function TestTypesProvider({ children }) {
  const [testTypes, setTestTypes] = useState(() => {
    try {
      const stored = typeof window !== 'undefined' ? localStorage.getItem('lims_test_types_v2') : null;
      if (stored) {
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') localStorage.setItem('lims_test_types_v2', JSON.stringify(testTypes));
  }, [testTypes]);

  return (
    <TestTypesContext.Provider value={{ testTypes }}>
      {children}
